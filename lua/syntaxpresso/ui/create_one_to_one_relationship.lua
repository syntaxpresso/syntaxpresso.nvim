...
local create_jpa_one_to_one_relationship = require("syntaxpresso.commands.create_jpa_one_to_one_relationship")
...
local function on_entity_type_select(_, node, _)
	...
	signal.owning_side_field_name = auto_field_name(node.type)
end
...
local function render_confirm_button()
	return n.button({
		...
		on_press = function()
			...
			create_jpa_one_to_one_relationship.create_jpa_one_to_one_relationship(
				relationship_config,
				function(response, error)
					...
					if response then
						vim.notify("One-to-One relationship created successfully!", vim.log.levels.INFO)
...
local function components(_previous_button_fn)
	return n.tabs(
		...
		-- Tab 1: Basic Configuration (Owning Side)
		n.tab(
			{ id = "basic_config" },
			n.rows(
				{ flex = 0 },
				render_main_title("One-to-One: Basic Configuration"),
				n.gap(1),
				...
				text_input.render_component({
					title = "Owning Side Field Name",
					signal = signal,
					signal_key = "owning_side_field_name",
					size = 1,
				}),
				text_input.render_component({
					title = "Inverse Side Field Name (bidirectional only)",
					signal = signal,
		...
		-- Tab 2: Inverse Side Configuration (only for bidirectional)
		n.tab(
			{ id = "inverse_config" },
			n.rows(
				{ flex = 0 },
				render_main_title("One-to-One: Inverse Side Configuration"),
				n.gap(1),
				select_many.render_component({
