...
---Create a JPA many-to-one relationship
...
function M.create_jpa_many_to_one_relationship(relationship_config, callback, options)
	...
	local fetch_type = get_signal_value(relationship_config.fetch_type)
	local collection_type = get_signal_value(relationship_config.collection_type)
	...
	-- Add optional fetch type
	if fetch_type and fetch_type ~= "" and fetch_type ~= "none" then
		args["fetch-type"] = string.lower(fetch_type)
	end

	-- Add optional collection type
	if collection_type and collection_type ~= "" then
		args["collection-type"] = string.lower(collection_type)
	end
	...
	-- Execute command
	command_runner.execute("create-jpa-many-to-one-relationship", args, callback, options)
end
