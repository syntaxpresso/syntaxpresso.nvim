...
While the plugin primarily uses `<leader>cj`, but you can bind specific commands directly, for example:
...
vim.keymap.set("n", "<leader>jR", "<cmd>SyntaxpressoCreateJpaRepositoryTui<CR>", { desc = "Create Repository" })
```
...
1. **Trigger**: You initiate an action (e.g., via Code Action).
